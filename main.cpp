...
bool LoginRegisterUI() {
    ...
    while (true) {
        ...
        settextstyle(40, 0, _T("黑体"));
        LPCTSTR loginText = _T("1. 登录账号");
        LPCTSTR registerText = _T("2. 注册账号");
        LPCTSTR exitText = _T("3. 退出游戏");

        outtextxy(swidth / 2 - textwidth(loginText) / 2, sheight / 5 * 2, loginText);
        outtextxy(swidth / 2 - textwidth(registerText) / 2, sheight / 5 * 2.5, registerText);
        outtextxy(swidth / 2 - textwidth(exitText) / 2, sheight / 5 * 3, exitText);
        ...
        cin.ignore();
        if (op == 1) {
            string account = ReadString("请输入账号: ");
            string password = ReadString("请输入密码: ", true);
            ...
            string account = ReadString("请输入新账号: ");
            if (account.empty()) {
                cout << "账号不能为空！" << endl;
                Sleep(1000);
                continue;
            }
            if (userDB.count(account)) {
                cout << "账号已存在！" << endl;
                Sleep(1000);
                continue;
            }
            string password = ReadString("请输入新密码: ", true);
            if (password.empty()) {
                cout << "密码不能为空！" << endl;
                Sleep(1000);
                continue;
